find_shortest_paths <- function(x, weight_){
  
  mst.probes <- dme2$ProbeName[which(dme2[[x]] !=0)]
  mst.tair <- mods$sseqid[mods$qseqid %in% mst.probes]
  test <- all_shortest_paths(gppi, from = which(V(gppi)$name %in% unique(hubs$TAIR)),
                             to = which(V(gppi)$name %in% mst.tair),mode = "all", weights = weight_)
  tl <- list()
  ...
  V(sp)$type <- "path"
  V(sp)$type[which(V(sp)$name %in% hubs$TAIR)] <- "root"
  V(sp)$type[which(V(sp)$name %in% mst.tair)] <- "ends"
